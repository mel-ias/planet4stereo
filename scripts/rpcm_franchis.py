...
"""
RPC functions adapted from:
title: "rpcm"
descriptiom: "Rational Polynomial Camera Model for optical satellite images."
URL: "https://github.com/centreborelli/rpcm'
version "1.4.9"
...
def get_z (path_DEM, lat, lon):
    with rasterio.open(path_DEM) as f:
        row,col = f.index(lon, lat)
        band1 = f.read(1)
        z = band1[row, col]
        print (lat, lon, z)
        return z
...
def angle_between_views(geotiff_path_1, geotiff_path_2, dem_path, lon=None, lat=None,
    ...
    if lat is None:
        lat = rpc1.lat_offset
    if z is None:
        z = get_z(path_DEM=dem_path, lon=lon, lat=lat)

    # debug
    #zenit1, azimuth1 = rpc1.incidence_angles(lon, lat, z)
    #print(geotiff_path_1, "zenit", zenit1, "azimuth", azimuth1 )
    #zenit2, azimuth2 = rpc2.incidence_angles(lon, lat, z)
    #print(geotiff_path_2, "zenit", zenit2, "azimuth", azimuth2 )

    # asterisk = unpack operation
    a = viewing_direction(*rpc1.incidence_angles(lon, lat, z))
    b = viewing_direction(*rpc2.incidence_angles(lon, lat, z))
